    ...
    it("should parse mrk format", function (done) {
        ...
        parse(data, { fromFormat: "mrk" }, function (err, records) {
            if (err) {
                ...
                expect(records[1].leader.marshal()).equal(
                    "01832cmmaa2200349 a 4500"
                );
            }
            done();
        });
        ...
        parse(
            ...
            function (err, records) {
                if (err) {
                    ...
                    expect(records[0].leader.marshal()).equal(
                        "01142cam  2200301 a 4500"
                    );
                    expect(records[0].dataFields[6].subfields[1].code).equal(
                        "d"
                    );
                }
                done();
            ...
            function (err, records) {
                ...
                transform(
                    ...
                    function (err, output) {
                        ...
                        expect(
                            output.indexOf("Premio del Público 27°")
                        ).to.be.least(0);
                        done();
                    }
                );
