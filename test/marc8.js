...
describe("marc8", function () {
                ...
                parse(
                    ...
                    function (err, records) {
                        ...
                        fields.forEach(function (field) {
                            if (field.tag == "240") {
                                expect(
                                    utf8.indexOf(field.subfields[0].data)
                                ).to.be.least(0);
                            } else if (field.tag == "008") {
                                expect(field.data).equal(
                                    "840112s1962 pau b 00010 eng"
                                );
                            }
                        });
                        done();
    ...
    //         parse(data, {fromFormat: 'iso2709', marc8: false, marc8converter: marc8 }, function(err, records) {
    ...
    it("should parse a file with mixed marc8 and utf8 encoding", function (done) {
        fs.readFile(
            "test/data/marc8_utf8_mixed.mrc",
            { encoding: "binary" },
            function (err, data) {
                parse(
                    data,
                    { fromFormat: "iso2709", marc8converter: marc8 },
                    function (err, records) {
